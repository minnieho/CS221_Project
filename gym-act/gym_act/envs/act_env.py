...
# For an example:
# cf https://github.com/openai/gym/blob/master/gym/envs/classic_control/cartpole.py
...
class IntelligentDriverModel():
	...
	def step(self, v_ego, v_oth=None, headway=np.Inf, dt=0.2):
...
class ActEnv(gym.Env):
	...
	def __init__(self, nobjs=2, driver_model='cv', max_accel=2, dist_collision=10, reward_shaping=False, discrete=False, pi_type='dnn',
		...
		self.dt = dt
		self.actions = actions # accel or decel command
	...
	def penalty_sa(self, state, a):
		...
		for n in range(self.nobjs):
			...
			accel = self.drivers[n].step(v_obj, dt=self.dt) # CALL driver model
	...
	def penalty_s(self, s):
		smallest_TTC, smallest_TTC_obj = get_smallest_TTC(s, self.dist_collision)
	...
	def _reward(self, s, a, sp):
		...
		self.smallest_TTC, self.smallest_TTC_obj = get_smallest_TTC(sp, self.dist_collision)
