	...
	for num_epoch in range(n_epochs):
		...
		for num_s, s in enumerate(mdp.train()):
			...
			for t in range(max_t):
				...
				s = sp
				if iters%100 == 99:
					print("Epoch no {}: sample {} iter {} avg_tr_loss: {:0.4f} tr_sliding_score: {:.2f}".format(num_epoch, num_s, iters, avg_tr_loss/100, mean_score))
					avg_tr_loss = 0
		...
		print("Epoch no {}: dev_mean_score: {:.2f}".format(num_epoch, dev_mean_score))
